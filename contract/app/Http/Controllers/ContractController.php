...
class ContractController extends Controller
{
    ...
    public function listWithStatus(){
        ...
        $contracts = Contract::query()->when(request('status') === 'Accepted' || request('status') === 'Pending', function ($query){

            $query->whereStatus('A')->orWhere('status','P')->orderBy('id','DESC');

        })->when(request('status') === 'Rejected', function($query){

            $query->whereStatus('R')->orderBy('id','DESC');

        })->when(request('status') === 'Closed', function ($query){

            $query->whereStatus('C')->orderBy('id','DESC');
